...
shape in the power spectrum (e.g. data with red noise) that has been rebinned (e.g.
the powers in every 3 frequency bins are summed).
...
normalised, and defined in linear space, so need to do pow_fit+ALOG10(0.5*cl95*MEAN(10.^(pow-pow_fit))/float(nbin)) and 
pow_fit+ALOG10(0.5*cl99*MEAN(10.^(pow-pow_fit))/float(nbin)) in IDL, where pow_fit is the fitted power law model.
...
#Read in number of bins that are summed over:
nbin = float(sys.argv[1])
...
def mypdf(w,x,sj):
	...
	return (np.exp(-0.5*logw*logw)*gammaincc(nbin,0.5*w*x))/(sq2pi*sj*w)
...
np.savetxt('cl95_rebin.dat', cl95, fmt="%f0")
np.savetxt('cl99_rebin.dat', cl99, fmt="%f0")
